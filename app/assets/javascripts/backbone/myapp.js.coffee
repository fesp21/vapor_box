...
jQuery ->
  ...
  Myapp.Views.StepView = Backbone.View.extend
    ...
    initializeShippingForm: ->
      ...
      @.StateField = @.$el.find('select[name=state]')
      ...
      @.shipStateField = @.$el.find('select[name=ship_state]')
    ...
    addressAttributes: ->
      if @.sameAddress.val()
        ...
        ship_state: @.shipStateField.find(':selected').val()
